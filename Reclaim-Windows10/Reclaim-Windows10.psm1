...
#    ... while retaining most win10 defaults + security features. My aim is to tweak Windows in a way suited to end-user rollout.
#
#    Now, my version is also written as a module. Once imported, you're able to call 
#	 * Reclaim-Windows10 for general use, or
# 	 * Reclaim-WindowsInfoSec for the original (dasm0) script, which seems to be more targeted at InfoSec pros.
#  
#    To install the module from the PowerShell gallery, I'm hoping you'll be able to run Install-Module -Name Reclaim-Windows10
#    This is my first commit, so let's see if it works!
#       
#    Please note in my version, I have also commented out the netflix, spotify, plex, twitter and facebook app uninstallers
#    Because I know some people use these. I've left the more egregious 3rd part bloat as being uninstalled.
#
#    Please also review the upstream @Disassembler0 script. It may be more up-to-date than mine:
...
<#
	.Synopsis
	Changes a lot of Windows 10 settings, with a view to reducing irritation, telemetry, and 3rd party bloat.
	...
	Scripts for reversing are included and commented. 
	Fork of https://github.com/Disassembler0/Win10-Initial-Setup-Script (different defaults)

	.Parameter Tweaks
	A replacement tweaks string array, that can be run in place of the usual defaults.

#>
Function Reclaim-Windows10() {
	param(
		[string[]]$Tweaks=@(
...
.Description
This is a PowerShell script for automation of routine tasks done after fresh installations of Windows 10 and Windows Server 2016. 
...
#>
Function Reclaim-WindowsInfoSec() {
	param(
		[string[]]$Tweaks=@(
			### Privacy Tweaks ###
			"DisableTelemetry",             # "EnableTelemetry",
...
function RunTweaks ($tweaks) {
	$IsAdmin = ([Security.Principal.WindowsPrincipal][Security.Principal.WindowsIdentity]::GetCurrent()).IsInRole([Security.Principal.WindowsBuiltInRole]"Administrator")
	If (!$IsAdmin) {
		Write-Error "You must run this module as a system administrator. Exiting."
	} 
	Else
	{
...
Function UnpinStartMenuTiles {
	Write-Output "Unpinning all Start Menu tiles... (Note: Log out to see changes)"
...
Function RequireAdmin {
	# No longer applicable in Powershell Module. 
	# Require on first line does the same job.
...
Export-ModuleMember -Function Reclaim-Windows10
Export-ModuleMember -Function Reclaim-WindowsInfoSec